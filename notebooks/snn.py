...
class Model:
    def __init__(self, debug=False):
        ...
        app['S_small_world'] = Synapses(app['EG'], app['EG'], stdp_small, on_pre=pre, on_post=post, method='euler', name='S_small_world')

        for (u, v) in G.edges():
            # print(f'{u} --- {v}')
            app['S_small_world'].connect(j=u, i=v)
            app['S_small_world'].w = 'rand()*gmax*0.1'  # здесь нужно сделать так, чтобы генерировались
    ...
    def train(self, X, num_of_examples, epoch=1):
        self.net['S1'].lr = 1
        ...
        for ep in tqdm(range(epoch)):
            ...
            for idx in range(len(X)):
                ...
                features[idx].append(np.array(mon.count, dtype=int8))
                # passive mode
...
def main():
    ...
    for matrix, label in sorted(zip(X_train, y_train), key=lambda x: x[1]):
        if selected_count[label] < 1:
    ...
    f_test = model.train(selected_matrices[:2], 1, 30)

    print(f_test)
    clf = RandomForestClassifier(max_depth=4, random_state=0)
    clf.fit(f_test[-1], y_test)

    print(clf.score(f_test[-1], y_test))
